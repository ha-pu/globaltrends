...
run_score <- function(control, object, lst_geo = lst_wdi) {
  ...
  purrr::map(lst_geo, ~{
	...
	qry_obj <- dplyr::filter(data_obj, batch == object & geo == .x)
        ...
        qry_map <- dplyr::mutate(qry_map, date = lubirdate::as_date(date))
        ...
        if (min(nrow(dplyr::count(qry_con, date)), nrow(dplyr::count(qry_obj, date)), nrow(dplyr::count(qry_map, date))) >= 24) {
          ...
          qry_con <- qry_con %>%
            ...
            tidyr::unnest(data) %>%
            dplyr::mutate(hits_trd = dplyr::case_when(hits_trd < 0 & hits_sad < 0 ~ 0.1,
                                        hits_trd < 0 ~ (hits_obs + hits_sad) / 2,
                                        TRUE ~ hits_trd),
                   hits_sad = dplyr::case_when(hits_sad < 0 & hits_trd < 0 ~ 0.1,
          ...
          qry_map <- qry_map %>%
            ...
            dplyr::mutate(data = purrr::map(data, .adjust_ts)) %>%
            tidyr::unnest(data) %>%
            dplyr::mutate(hits_trd = dplyr::case_when(hits_trd < 0 & hits_sad < 0 ~ 0,
                                        hits_trd < 0 ~ (hits_obs + hits_sad) / 2,
                                        TRUE ~ hits_trd),
                   hits_sad = dplyr::case_when(hits_sad < 0 & hits_trd < 0 ~ 0,
                                        hits_sad < 0 ~ (hits_obs + hits_trd) / 2,
                                        TRUE ~ hits_sad)
            )
        }
        ...
        qry_con <- qry_map %>%
          ...
          dplyr::mutate(value_m = dplyr::case_when(value_m == 0 ~ 1, TRUE ~ value_m),
        ...
        data_obj_agg <- qry_obj %>%
          ...
          dplyr::mutate(score = dplyr::coalesce(value / value_c, 0),
