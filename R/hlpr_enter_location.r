...
.enter_location <- function(doiGT_DB) {
  ...
  lst_wdi <- select(lst_wdi, location = iso2c)
  lst_wdi <- WDI(country = lst_wdi$location, indicator = "NY.GDP.MKTP.KD", start = 2018, end = 2018)
  ...
  lst_wdi <- select(lst_wdi, location = iso2c, name = country, share, cum_share)
  ...
  lst_usa <- select(lst_usa, location = sub_code, name)
